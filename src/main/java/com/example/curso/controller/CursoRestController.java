...
public class CursoRestController {
	...
	public ResponseEntity<?> listaCursos() {
		List<CursoEntity> listaCursos = cursoService.findAll();
		if(listaCursos!=null) {
	...
	public ResponseEntity<?> agregarCurso(@RequestBody CursoEntity curso) {
		cursoService.saveCurso(curso);
		return new ResponseEntity<Void>(HttpStatus.CREATED);
	}
	
	@PostMapping("/cursos_profesor")
	public ResponseEntity<?> verCursosProfesor(@RequestBody ProfesorEntity profesor) {
		List<CursoEntity> listaCursos = cursoService.getCursosProfesor(profesor.getId());
		if(listaCursos!=null) {
			if(listaCursos.size()!=0) {
				return new ResponseEntity<>(listaCursos, HttpStatus.OK);
			} else {
